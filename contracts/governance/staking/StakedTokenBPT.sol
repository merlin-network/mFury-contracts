...
contract StakedTokenBPT is StakedToken {
    ...
    event FeesConverted(uint256 bpt, uint256 fury);
    ...
    ****************************************/
     ...
     * @param _rewardsToken Token that is being distributed as a reward. eg Meta (FURY)
     * @param _questManager Centralised manager of quests
     * @param _stakedToken Core token that is staked and tracked e.g. mFury FURY/WETH Staking BPT (mBPT)
    ...
    ****************************************/
    ...
    /**
     * @dev Converts redemption fees accrued in mBPT into FURY, before depositing to the rewards contract.
     */
    function convertFees() external nonReentrant {
        ...
        uint256 furyBalBefore = REWARDS_TOKEN.balanceOf(address(this));

        (address[] memory tokens, , ) = balancerVault.getPoolTokens(poolId);
        require(tokens[0] == address(REWARDS_TOKEN), "not FURY");
        ...
        // 1.3. Exits rewards (FURY) to this staking contract for mBPT from this staking contract.
        // Assumes rewards token (FURY) is in position 0
        ...
        // 3. Inform HeadlessRewards about the new FURY rewards
        uint256 received = REWARDS_TOKEN.balanceOf(address(this)) - furyBalBefore;
        require(received >= minOut[0], "< min FURY");
    ...
    ****************************************/
     ...
     * PriceCoeff = units of FURY per BPT, scaled to 1:1 = 10000
     * Assuming an 80/20 BPT, it is possible to calculate
     * PriceCoeff (p) = balanceOfFURY in pool (b) / bpt supply (s) / 0.8
     * p = b * 1.25 / s
     */
    function getProspectivePriceCoefficient() public view returns (uint256 newPriceCoeff) {
        ...
        // Calculate units of FURY per BPT
