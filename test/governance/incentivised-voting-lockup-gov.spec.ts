...
describe("IncentivisedVotingLockup", () => {
    ...
    const fundVotingLockup = async (funding = simpleToExactAmount(100, DEFAULT_DECIMALS)) => {
        await fury.connect(sa.fundManager.signer).transfer(votingLockup.address, funding)
    ...
    const deployFresh = async (initialRewardFunding = BN.from(0)) => {
        ...
        fury = await new MintableToken__factory(sa.default.signer).deploy(nexus.address, sa.fundManager.address)
        await fury.connect(sa.fundManager.signer).transfer(sa.default.address, simpleToExactAmount(1000, DEFAULT_DECIMALS))
        await fury.connect(sa.fundManager.signer).transfer(sa.other.address, simpleToExactAmount(1000, DEFAULT_DECIMALS))
        ...
        await fury.approve(votingLockup.address, simpleToExactAmount(100, DEFAULT_DECIMALS))
        await fury.connect(sa.other.signer).approve(votingLockup.address, simpleToExactAmount(100, DEFAULT_DECIMALS))
        await fury.connect(sa.fundManager.signer).approve(votingLockup.address, simpleToExactAmount(10000, DEFAULT_DECIMALS))
    ...
    const snapshotData = async (sender = sa.default): Promise<ContractData> => {
        ...
        return {
            ...
            senderStakingTokenBalance: await fury.balanceOf(sender.address),
            contractStakingTokenBalance: await fury.balanceOf(votingLockup.address),
    ...
    describe("performing full system flow", () => {
        ...
        before(async () => {
            ...
            await fury.connect(sa.fundManager.signer).transfer(alice.address, simpleToExactAmount(1, 22))
            await fury.connect(sa.fundManager.signer).transfer(bob.address, simpleToExactAmount(1, 22))
            await fury.connect(sa.fundManager.signer).transfer(charlie.address, simpleToExactAmount(1, 22))
            await fury.connect(sa.fundManager.signer).transfer(david.address, simpleToExactAmount(1, 22))
            await fury.connect(sa.fundManager.signer).transfer(eve.address, simpleToExactAmount(1, 22))
            await fury.connect(alice.signer).approve(votingLockup.address, simpleToExactAmount(100, 21))
            await fury.connect(bob.signer).approve(votingLockup.address, simpleToExactAmount(100, 21))
            await fury.connect(charlie.signer).approve(votingLockup.address, simpleToExactAmount(100, 21))
            await fury.connect(david.signer).approve(votingLockup.address, simpleToExactAmount(100, 21))
            await fury.connect(eve.signer).approve(votingLockup.address, simpleToExactAmount(100, 21))
        })
        describe("checking initial settings", () => {
            ...
            it("sets ERC20 details", async () => {
                ...
                expect(name).eq("Voting FURY")
                expect(symbol).eq("vFURY")
    ...
    describe("testing voting powers changing", () => {
        ...
        it("calculates voting weights on a rolling basis", async () => {
            ...
            await fury.connect(sa.fundManager.signer).transfer(alice.address, amount.mul(5))
            await fury.connect(sa.fundManager.signer).transfer(bob.address, amount.mul(5))
            const stages = {}

            await fury.connect(alice.signer).approve(votingLockup.address, amount.mul(5))
            await fury.connect(bob.signer).approve(votingLockup.address, amount.mul(5))
            ...
            const aliceBalBefore = await fury.balanceOf(alice.address)
            const bobBalBefore = await fury.balanceOf(bob.address)
            ...
            expect(await fury.balanceOf(alice.address)).eq(aliceBalBefore.add(aliceRewardsEarned2.sub(aliceRewardsEarned1)))
            expect(await fury.balanceOf(bob.address)).eq(bobBalBefore.add(bobRewardsEarned))
