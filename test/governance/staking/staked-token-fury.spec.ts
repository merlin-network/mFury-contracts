...
describe("Staked Token FURY rewards", () => {
    let sa: StandardAccounts
    let nexus: MockNexus
    let rewardToken: MockERC20
    let stakedToken: StakedTokenFURY
    let rewardsVendorAddress: string
    // const startingMintAmount = simpleToExactAmount(10000000)

    console.log(`Staked Token FURY contract size ${StakedTokenFURY__factory.bytecode.length / 2} bytes`)
    ...
    const redeployStakedToken = async (): Promise<void> => {
        ...
        const stakedTokenFactory = new StakedTokenFURY__factory(stakedTokenLibraryAddresses, sa.default.signer)
    ...
    context("collecting fees in $FURY", () => {
