...
const furyWhaleAddress = "0x24167305A3667023Ea565f971D72509ef758Ac78"
...
context("StakedToken deployments and vault upgrades", () => {
    ...
    const setup = async (blockNumber?: BigNumberish) => {
        ...
        furyWhale = await impersonateAccount(furyWhaleAddress)
        mbptWhale = await impersonateAccount(mbptWhaleAddress)

        mBPT = IERC20__factory.connect(resolveAddress("mBPT"), deployer.signer)
        FURY = IERC20__factory.connect(resolveAddress("FURY"), furyWhale.signer)
    ...
    context("1. Upgrade", () => {
        ...
        describe("post upgrade verification", () => {
            ...
            it("StakedToken config", async () => {
                ...
                expect(configAfter.rewardToken, "reward token symbol").eq(resolveAddress("FURY"))
    ...
    describe("upgrade and withdraw", () => {
        ...
        it("convert fees to FURY", async () => {
            const furyBalBefore = await FURY.balanceOf(stkBPT.address)
            expect(furyBalBefore, "stkBPT's FURY bal before").to.gt(0)
            const tx = await stkBPT.connect(deployer.signer).convertFees()

            await expect(tx).to.emit(stkBPT, "FeesConverted")

            expect(await FURY.balanceOf(stkBPT.address), "stkBPT's FURY bal after").to.gt(furyBalBefore)
        })
