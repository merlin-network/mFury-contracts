...
import { StakedTokenBPT__factory, StakedTokenFURY__factory, StakedToken__factory, StakedTokenBatcher__factory } from "types/generated"
...
subtask("staked-snap", "Dumps a user's staking token details.")
    .addOptionalParam("asset", "Symbol of staking token. FURY or mBPT", "FURY", types.string)
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        const priceCoeff = taskArgs.asset === "FURY" ? BN.from(10000) : await stakingToken.priceCoefficient()
        ...
        console.log(`FURY Price coefficient ${formatUnits(priceCoeff, 4)}`)
...
subtask("staked-stake", "Stake FURY or mBPT in V2 Staking Token")
    .addOptionalParam("asset", "Symbol of staking token. FURY or mBPT", "FURY", types.string)
...
subtask("staked-withdraw", "Withdraw FURY or mBPT in V2 Staking Token")
    .addOptionalParam("asset", "Symbol of staking token. FURY or mBPT", "FURY", types.string)
    .addParam("amount", "Amount to of token to be staked without the token decimals.", undefined, types.float)
...
subtask("staked-claim", "Claim FURY rewards from V2 staking token")
    .addOptionalParam("recipient", "Address or contract name that will receive the FURY rewards.", undefined, types.string)
    .addOptionalParam("speed", "Defender Relayer speed param: 'safeLow' | 'average' | 'fast' | 'fastest'", "average", types.string)
    .setAction(async (taskArgs, hre) => {
        const signer = await getSigner(hre, taskArgs.speed, false)
        const chain = getChain(hre)

        const stakingTokenAddress = resolveAddress("FURY", chain, "vault")
        const stakingToken = StakedToken__factory.connect(stakingTokenAddress, signer)
        let tx
        ...
        const receipt = await logTxDetails(tx, `Claim earned FURY rewards`)
        console.log(`Claimed ${formatUnits(receipt.events[0].args[2])} FURY rewards`)
    })
task("staked-claim").setAction(async (_, __, runSuper) => {
    await runSuper()
})

subtask("staked-compound", "Stake any earned FURY rewards")
    .addOptionalParam("speed", "Defender Relayer speed param: 'safeLow' | 'average' | 'fast' | 'fastest'", "average", types.string)
    .setAction(async (taskArgs, hre) => {
        const signer = await getSigner(hre, taskArgs.speed, false)
        const chain = getChain(hre)
        ...
        const stakingTokenAddress = resolveAddress("FURY", chain, "vault")
        const stakingToken = StakedTokenFURY__factory.connect(stakingTokenAddress, signer)
        const tx = await stakingToken.compoundRewards()
        const receipt = await logTxDetails(tx, "Stake earned FURY rewards")
        console.log(`Staked ${formatUnits(receipt.events[0].args[2])} FURY rewards`)
    })
...
subtask("staked-fees", "Converts fees accrued in BPT to FURY, before depositing to the rewards contract.")
    .addOptionalParam("speed", "Defender Relayer speed param: 'safeLow' | 'average' | 'fast' | 'fastest'", "average", types.string)
    .setAction(async (taskArgs, hre) => {
        const signer = await getSigner(hre, taskArgs.speed, false)
        const chain = getChain(hre)
        ...
        if (feesBPT.lt(simpleToExactAmount(100))) {
            console.log(`Only ${feesBPT} mBPT in fees so will not convert to FURY`)
            return
        }
...
subtask("staked-time-all-users", "Updates all user's time multiplier.")
    .addOptionalParam("assets", "Symbol of staking token. FURY or mBPT", "FURY,mBPT", types.string)
    .addOptionalParam("speed", "Defender Relayer speed param: 'safeLow' | 'average' | 'fast' | 'fastest'", "safeLow", types.string)
    .setAction(async (taskArgs, hre) => {
        const chain = getChain(hre)
        const signer = await getSigner(hre, taskArgs.speed, false)
