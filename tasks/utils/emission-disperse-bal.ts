...
export const fetchBalancerReport = async (report: number): Promise<Array<{ address: string; amount: string }>> => {
    const url = `${REPORT_URL}/${report}/__polygon_${PFURY.address}.json`
...
/**
 * Gets the rewards values to be disperse via DisperserForwarder of a given report, the values are scaled to match the total FURY balance.
 * @dev The Polygon FURY rewards for  will be in the __polygon_0xF501dd45a1198C2E1b5aEF5314A68B9006D842E0.json file under the report folder with a week number.
 ...
 *  The amounts in this file assumes 15k FURY is being distributed but this will not be the case with the emissions controller.
 *  Need to proportion the FURY balance in the DisperseForwarder contract to the recipients based off the bal-mining-script report.
 *
 * @param {number} report - Report number from the bal-mining-script repo. eg 79
 *  https://github.com/balancer-labs/bal-mining-scripts/blob/master/reports/WEEK
 * @param {BN} furyBalance - The total amount of fury to disperse, values on the report are scaled to match the total amount of FURY available
 * @return {Promise}  {Promise<BalancerPolygonReport>}
 */
export const getBalancerPolygonReport = async (report: number, furyBalance: BN): Promise<BalancerPolygonReport> => {
    ...
    balancerRewards.forEach((reward) => {
        ...
        const disperseRewardAmount = simpleToExactAmount(reward.amount, 18).mul(furyBalance).div(balancerTotal)
    ...
    log(`total fury token amount to disperser[${disperseTotal.toString()}], total recipients count [${disperseRecipients.length}]`)
    return {
